...
function activityHTML(activity) {
  ...
  $article.append($("<h5>").append("Program:&nbsp;").append($("<a>").attr("href",activity['programHomePage']).attr("target","_blank").append(`${activity['programShortName']}`)));
  $article.append($("<h5 class='metrics-link'>").append($("<a>").attr('href',metricsLink).attr('target','_blank').append('Activity Metrics')));
  $article.append($("<a>").attr("href",toStateUrl(activity['state'])).attr("target","_blank").append($("<img>").attr("class","activity-state-badge").attr("src",`https://cdn.jsdelivr.net/gh/finos/contrib-toolbox@master/images/badge-${activity['state'].toLowerCase()}.svg`)));
